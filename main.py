...
class App:
	def __init__(self):
		...
		self.root.title("心电三级智能检测系统")
		self.root.resizable(False, False)          # 设置窗体不可改变大小
		self.no_title = True
		self.show_title()
		self.body()
#		self.root.attributes("-toolwindow", 1)
##		self.root.attributes('-lowmost', True)
	def body(self):
		# ---------------------------------------------------------------------
		# 背景图片
		# ---------------------------------------------------------------------
		...
		im1 = tku.image_label(f1, "images\\心电.gif", 86, 86, False)
		...
		tk.Label(f1, text="心电三级智能检测系统", height=2, fg="white", font=ft1, bg="Teal")\
		...
		tk.Button(canvas, text="患者系统", bg="cadetblue", command=self.show_ai, font=ft2, height=2, fg="white", width=15)\
			.pack(side=tk.LEFT, expand=tk.YES, anchor=tk.CENTER, padx=5)
		tk.Button(canvas, text="医生系统", bg="cadetblue", command=self.show_patient, font=ft2, height=2, fg="white", width=15)\
	...
	def show_ai(self):
		ai1.Window1(self.root)

	def show_patient(self):
		patient1.patient(self.root)
